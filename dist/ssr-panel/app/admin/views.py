...
def addport():
    form = AddportForm()
    if form.validate_on_submit():
        if form.data['port'].strip(" ") == "" or form.data['pwd'].strip(" ") == "":
            flash("端口或密码不合法","error")
            return redirect(url_for("admin.portlist"))
        ssr = ssrlist()
        ssr['port_password'][form.data['port']] = form.data['pwd'] #update ssr record
        json.dump(ssr,open("/etc/shadowsocks/config.json" ,"w"),ensure_ascii=False)
...
def delport(port):
    ...
    try:
        ...
        flash("端口错误","error")
    json.dump(ssrdata,open("/etc/shadowsocks/config.json" ,"w"),ensure_ascii=False)
    os.system("/etc/init.d/shadowsocks restart")
    return redirect(url_for('admin.portlist'))
